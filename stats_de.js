...
const typeLabels = {
    ...
    'D.Ip.': 'Dringliche Interpellation',
    'BRG': 'Geschäft des BR'
};
...
const typeToFilter = {
    ...
    'Fragestunde': 'Fra.',
    'Anfrage': 'A.',
    'Parl. Initiative': 'Pa. Iv.',
    'Dringliche Interpellation': 'D.Ip.',
    'Geschäft des BR': 'BRG'
};
...
// Construit l'URL vers debates_de.html avec tous les filtres actifs + un filtre additionnel
function buildDebatesUrl(additionalFilter = {}) {
    ...
    const queryString = params.toString();
    return `debates_de.html${queryString ? '?' + queryString : ''}`;
}
...
function updateGlobalSummary() {
    ...
    if (pctEluEl && pctCFEl && globalFilteredObjects.length > 0) {
        ...
        if (bothNoteEl && both > 0) {
            bothNoteEl.textContent = `davon ${both} von beiden`;
        }
    }
...
// Plugin für Pulsationseffekt auf Punkten
const pulsePlugin = {
    id: 'pulseEffect',
    afterDraw: (chart) => {
        const ctx = chart.ctx;
