...
async function init() {
    try {
        // Debatten, Objekte und fehlende Tags parallel laden
        ...
        // Tags der fehlenden Objekte hinzufügen (nicht in cdf_efk_data.json)
        if (missingTagsJson.items) {
            missingTagsJson.items.forEach(item => {
                if (item.business_number && !objectsData[item.business_number]) {
                    // Utiliser tags_de, ou tags FR en fallback
                    objectsData[item.business_number] = item.tags_de || item.tags || '';
                }
            });
        }
...
function applyFilters() {
    ...
    filteredData = allData.filter(item => {
        // New updates filter (< 4 Tage, kohärent mit grünem Band)
        if (window.newUpdatesFilter) {
            const now = new Date();
...
function createCard(item, searchTerm = '') {
    ...
    if (item.business_number && typeof handleSummaryClick === 'function' && typeof isLLMAvailable === 'function' && isLLMAvailable()) {
        const locale = typeof getLocale === 'function' ? getLocale() : { btnText: '🤖 Geschäft zusammenfassen', btnTitle: 'KI-Zusammenfassung erstellen' };
